...
date: "5/05/2023" #date finished working on it
output: 
  html_document:
    toc: true
    toc_float: true
...
## Load data
**Note:** Filepaths will not be shown in knitted version
...
# Preliminary Exploration of Bivariate Relationships
...
Test association between sequencing batch and adversity variables, and numbers of each adversity group in each batch
```{r eval=FALSE}
...
```
Conclusion: adversity grouping variable prevalences do not differ by sequencing batch, and are not associated with sequencing batch 
...
# create a dataset with only the participants you are including in the main analysis
...
### Figure 3: Differentially Abundant Taxa as a Function of Adversity-Correlated Child Socioemotional Functioning Domains
...
- postnatal adversity & parabacteriodes (all adversities in model; n.s. when covariates not included)
...
Here we will only test prenatal adversity (only adversities in model), developmental problems at Y2, and sleep problems at Y4 because we found taxa from the same family to be differentially abundant in those domains and these analyses are specifically to follow-up on those results. 
